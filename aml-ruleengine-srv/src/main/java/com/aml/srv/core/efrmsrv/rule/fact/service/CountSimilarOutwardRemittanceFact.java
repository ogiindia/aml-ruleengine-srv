...
@Service("COUNT_SIMILAR_OUTWARD_REMITTANCESService")
public class CountSimilarOutwardRemittanceFact implements FactInterface{


	private Logger LOGGER = LoggerFactory.getLogger(CountSimilarOutwardRemittanceFact.class);
	@Autowired
	...
	public ComputedFactsVO getFactExecutor(RuleRequestVo requVoObjParam, Factset factSetObj,
		...
		LOGGER.info("REQID : [{}]::::::::::::CountSimilarOutwardRemittanceFact@getFactExecutor (ENTRY) Called::::::::::",
		...
		try {
			...
			/*TransactionDetailsDTO dto = transactionService.getTransactionDetails(reqId, custId, accNo, txnId, null,
					AMLConstants.DEPOSIT, transMode, days, months, factSetObj, range);*/
			...
			LOGGER.error("Exception found in CountSimilarOutwardRemittanceFact@getFactExecutor : {}", e);
		} finally {dto = null; transSrvSrchFilevoObj = null;

			LOGGER.info("REQID : [{}]::::::::::::CountSimilarOutwardRemittanceFact@getFactExecutor (EXIT) End::::::::::\n\n", requVoObjParam.getReqId());
		}
